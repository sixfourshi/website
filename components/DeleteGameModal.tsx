      ...
      <div className="w-full max-w-lg rounded-2xl border border-red-500/40 bg-[#0d0f24] p-6 shadow-2xl">
        ...
        <div className="mt-5 rounded-xl border border-line/80 bg-[#060b1d] p-4 text-xs text-slate-300">
          ...
          <p className="text-slate-300 leading-relaxed">
            There are currently{' '}
            <strong className="text-azure-300">{associatedScriptCount} script(s)</strong> tagged with this game. How would you like to handle them?
          </p>
        </div>
          ...
          >
            ...
            <div>
              <div className="font-semibold text-xs text-red-300">
                Permanently delete associated scripts
              </div>
              <div className="text-[11px] text-slate-400 mt-0.5 leading-snug">
                Deletes this game and removes all {associatedScriptCount} associated script(s) from the system.
              </div>
            </div>
          </label>
        </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-3 border-t border-line/60 pt-4">
          <button
            type="button"
            onClick={onClose}
            disabled={deleting}
            className="rounded-xl border border-line px-4 py-2.5 text-xs font-medium text-slate-300 hover:text-white hover:bg-white/5 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            disabled={deleting}
            className="rounded-xl bg-red-600 px-5 py-2.5 text-xs font-semibold text-white shadow-lg hover:bg-red-500 disabled:opacity-50 transition-all cursor-pointer"
          >
