...
void main() {
  ...
  if (uRadius > 0.0) {
    ...
    alpha = 1.0 - smoothstep(-1.25, 1.25, sd);
...
export default function ElasticMesh({
  ...
  useEffect(() => {
    ...
    function resize() {
      if (!container) return;
      const w = container.offsetWidth || window.innerWidth || 1;
      const h = container.offsetHeight || window.innerHeight || 1;
    ...
    function toPlane(clientX: number, clientY: number) {
      ...
      const mx = (clientX - rect.left) / width;
      const my = (clientY - rect.top) / height;
      const clipX = mx * 2 - 1;
      const clipY = 1 - my * 2;
      const t = ((propsRef.current.tilt || 0) * Math.PI) / 180;
      const ct = Math.cos(t);
      const st = Math.sin(t);
      const activeFit = propsRef.current.fit ?? DEFAULT_FIT;
      const a = clipY / (ct * activeFit * DIST);
      const py = (a * DIST) / (1 + a * st);
      const persp = DIST / (DIST - py * st);
      pointer.tx = (clipX * aspect) / (persp * activeFit);
      pointer.ty = py;
    }
    ...
    function frame(now: number) {
      ...
      program.uniforms.uFit.value = p.fit ?? DEFAULT_FIT;
