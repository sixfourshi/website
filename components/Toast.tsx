...
export function showToast(
  ...
  const event = new CustomEvent(TOAST_EVENT, {
    detail: { message, type, duration, subtitle },
  });
...
export async function copyToClipboard(
  ...
  try {
    if (navigator.clipboard && window.isSecureContext) {
      ...
      showToast(successMessage, 'success', 3000, 'Ready to paste into your executor');
      return true;
    } else {
    ...
    showToast('Failed to copy script to clipboard.', 'error', 3500, 'Clipboard permission was blocked');
    return false;
  }
}
...
export function ToastContainer() {
  ...
  return (
    <div
      aria-live="polite"
      className="fixed bottom-5 left-1/2 -translate-x-1/2 sm:bottom-6 sm:right-6 sm:left-auto sm:translate-x-0 z-50 pointer-events-none w-[92vw] max-w-md sm:w-auto"
    >
      <div
